...
const EXCLUDED_TABLES = new Set(["tblBrunelDay", "tblDataTypeDef", "tblMostRecentRate", "tblTempInvoice"]);
...
async function convert(opts: ConvertOptions): Promise<void> {
  ...
  const allNames = tables ?? getTableNames(accessDb);
  const tableNames = allNames.filter((n) => !EXCLUDED_TABLES.has(n));
