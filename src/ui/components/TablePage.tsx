...
const CUSTOM_SORT: Record<string, string[]> = {
  rates:        ["ClientID", "Month"],
  expenseItems: ["ClientID", "Date"],
};
...
export function TablePage({ apiRoute }: Props) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      let options: Record<string, { value: string | number; label: string }[]> = {};
      ...
      const sortKeys = CUSTOM_SORT[apiRoute] ?? [cfg.labelColumn];
      data.data.sort((a, b) => {
        for (const key of sortKeys) {
          const fkMap = labels[key];
          const av = fkMap ? (fkMap[String(a[key])] ?? "") : String(a[key] ?? "");
          const bv = fkMap ? (fkMap[String(b[key])] ?? "") : String(b[key] ?? "");
