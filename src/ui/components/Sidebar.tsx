...
export function Sidebar({ configs, current, onSelect }: Props) {
  ...
  async function handleDownload(e: React.FormEvent) {
    ...
    try {
      ...
      const filename = res.headers.get("X-Filename") ?? "clientinvoice-backup.sqlite.enc";
  ...
  return (
    <nav className="w-60 bg-slate-950 flex flex-col flex-shrink-0 border-r border-slate-800">
      ...
      <div className="px-6 pt-7 pb-6">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-indigo-600 flex items-center justify-center shadow-lg shadow-indigo-600/20">
            <span className="text-white text-sm font-bold font-display">CI</span>
          </div>
          <div>
      ...
      {/* Nav items */}
      <div className="flex flex-col px-3 gap-0.5 flex-1 overflow-y-auto scroll-thin">
        {configs.map((cfg) => {
      ...
      <div className="px-4 py-5 border-t border-slate-800/60 flex flex-col gap-2">
        ...
        <button
          onClick={() => {
            if (!window.confirm("Shut down Client Invoice?")) return;
            setShutdown(true);
      ...
      {shutdown && (
        <div className="fixed inset-0 bg-slate-950 flex items-center justify-center z-[999]">
          <div className="text-center">
            ...
            <p className="text-slate-400 text-sm">Client Invoice has been shut down.</p>
